...
from gpt_image_mcp.config.settings import Settings, StorageSettings, CacheSettings, ImageSettings
...
async def storage_manager(mock_storage_settings) -> AsyncGenerator[ImageStorageManager, None]:
...
def mock_openai_client_manager(mock_openai_settings, mock_openai_client):
    ...
    with patch.object(OpenAIClientManager, '_create_client', return_value=mock_openai_client):
