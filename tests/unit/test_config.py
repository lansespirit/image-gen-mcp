...
class TestStorageSettings:
    ...
    def test_storage_settings_validation(self):
        ...
        # Valid settings
        settings = StorageSettings(retention_days=1, max_size_gb=1, cleanup_interval_hours=1)
...
class TestCacheSettings:
    ...
    def test_cache_settings_validation(self):
        ...
        for backend in ["memory", "redis"]:
            settings = CacheSettings(backend=backend)
...
class TestCompleteSettings:
    ...
    def test_settings_from_dict(self):
        ...
        assert settings.storage.base_path == "/tmp/test"
...
DEFAULT_QUALITY=high
    ...
    def test_settings_validation_cascade(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            Settings(
                openai=OpenAISettings(api_key=""),  # Invalid empty key
                server=ServerSettings(port=70000)  # Invalid port
            )
