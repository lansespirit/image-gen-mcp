    ...
    def _load_templates(self) -> None:
        ...
        try:
            ...
            for template_id, template_data in data.get("templates", {}).items():
                self.templates[template_id] = self._parse_template(template_id, template_data)
...
class TemplateRenderer:
    ...
    def render(self, template_id: str, **kwargs) -> Tuple[str, TemplateMetadata]:
        ...
        for param_name, param in template.parameters.items():
            if param.required and param_name not in kwargs:
                raise ValueError(f"Required parameter '{param_name}' missing for template '{template_id}'")
    ...
    def _apply_conditional_parts(self, template: Template, kwargs: Dict[str, Any]) -> None:
    ...
    def validate_parameters(self, template_id: str, parameters: Dict[str, Any]) -> List[str]:
        ...
        for param_name, value in parameters.items():
            ...
            if param.max_length and isinstance(value, str) and len(value) > param.max_length:
                errors.append(f"Parameter '{param_name}' exceeds maximum length of {param.max_length}")
