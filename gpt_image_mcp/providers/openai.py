...
class OpenAIProvider(LLMProvider):
    ...
    async def generate_image(
        ...
        **kwargs
        ...
        if size in capabilities.supported_sizes:
            ...
            self._logger.warning(f"Size '{size}' not supported, using {request_params['size']}")
    ...
    def estimate_cost(self, model: str, prompt: str, image_count: int = 1) -> Dict[str, any]:
        ...
        if model == "gpt-image-1":
            ...
            text_cost = (text_tokens / 1_000_000) * model_pricing["text_input_per_1m_tokens"]
            image_cost = (image_count * model_pricing["tokens_per_image"] / 1_000_000) * model_pricing["image_output_per_1m_tokens"]
