...
from .base import LLMProvider, ProviderConfig, ImageResponse, ModelCapability, ProviderError

logger = logging.getLogger(__name__)


class GeminiProvider(LLMProvider):
    """Gemini provider for image generation using Imagen models via OpenAI compatibility."""
    ...
    SUPPORTED_MODELS = {
        ...
        "imagen-4-ultra": ModelCapability(
            ...
            supported_qualities=["auto", "high", "medium", "low"],
            supported_formats=["png", "jpeg", "webp"],
            max_images_per_request=1,  # Imagen 4 Ultra can only generate one image at a time
    ...
    async def generate_image(
        ...
        **kwargs
        ...
        if style != "vivid":
            self._logger.info(f"Style parameter '{style}' is not supported by Gemini, ignoring")
        if background != "auto":
            self._logger.info(f"Background parameter '{background}' is not supported by Gemini, ignoring")
    ...
    def validate_model_params(self, model: str, params: Dict[str, any]) -> Dict[str, any]:
        ...
        if "size" in params:
            ...
            if aspect_ratio not in capabilities.custom_parameters.get("aspect_ratio", []):
                # Use default aspect ratio
                params["size"] = "1024x1024"  # Maps to 1:1
                self._logger.warning(f"Aspect ratio not supported, using 1:1 (1024x1024)")
                
        # Validate image count for Imagen 4 Ultra
        if model == "imagen-4-ultra" and params.get("n", 1) > 1:
            params["n"] = 1
            self._logger.warning("Imagen 4 Ultra only supports generating 1 image at a time")
            
        # Remove unsupported parameters
        unsupported_params = ["style", "background", "moderation"]
        for param in unsupported_params:
            if param in params:
                self._logger.debug(f"Removing unsupported parameter '{param}' for Gemini")
                del params[param]
