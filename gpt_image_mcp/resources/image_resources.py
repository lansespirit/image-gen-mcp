        ...
        try:
            ...
            for image_metadata in recent_images:
                formatted_image = {
                    ...
                    "prompt": image_metadata.get("prompt", "")[:100] + "..." if len(image_metadata.get("prompt", "")) > 100 else image_metadata.get("prompt", ""),
                    "resource_uri": f"generated-images://{image_metadata.get('image_id')}",
                    "file_size_bytes": image_metadata.get("file_info", {}).get("size_bytes"),
                    ...
                    "cost_estimate": image_metadata.get("cost_info", {}).get("estimated_cost_usd"),
                }
        ...
        try:
            ...
            enhanced_stats = {
                **stats,
                ...
                "storage_health": {
                    "status": "healthy" if stats.get("storage_usage_mb", 0) < (self.settings.max_size_gb * 1024 * 0.9) else "near_limit",
                    "usage_percentage": round((stats.get("storage_usage_mb", 0) / (self.settings.max_size_gb * 1024)) * 100, 2),
                },
                "recommendations": [],
            }
            ...
            if usage_pct > 90:
                enhanced_stats["recommendations"].append("Storage usage is high. Consider cleaning up old files or increasing storage limit.")
            elif usage_pct > 75:
                enhanced_stats["recommendations"].append("Storage usage is moderate. Monitor usage and consider cleanup policies.")
            
            if stats.get("total_images", 0) == 0:
                enhanced_stats["recommendations"].append("No images found. Start generating images to populate storage.")
