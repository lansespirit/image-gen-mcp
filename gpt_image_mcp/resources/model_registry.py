...
class ModelRegistry:
    ...
    async def get_model_info(self, model_id: str) -> Optional[ModelInfo]:
        ...
        if model_file.exists():
            try:
                async with aiofiles.open(model_file, 'r', encoding='utf-8') as f:
    ...
    async def get_model_documentation(self, model_id: str) -> str:
        ...
        Returns:
            Formatted markdown documentation (includes 'not found' message if model doesn't exist)
        """
...
"""
        ...
        doc += f"\n## Quality Levels\n"
        doc += f"{chr(10).join(f'- {quality}' for quality in model_info.quality_levels)}\n"
        ...
        if model_info.best_practices:
            doc += "\n## Best Practices\n"
            doc += f"{chr(10).join(f'- {practice}' for practice in model_info.best_practices)}\n"
        
        if model_info.examples:
            doc += "\n## Examples\n"
            doc += f"{chr(10).join(f'- {example}' for example in model_info.examples)}\n"
