...
class ImageGenerationParams(BaseModel):
    ...
    quality: ImageQuality = Field(
        default=ImageQuality.AUTO, description="Image quality level"
    )

    size: ImageSize = Field(default=ImageSize.LANDSCAPE, description="Image dimensions")

    style: ImageStyle = Field(
        default=ImageStyle.VIVID, description="Image style preference"
    )

    moderation: ModerationLevel = Field(
        default=ModerationLevel.AUTO, description="Content moderation level"
    )
    ...
    compression: int = Field(
        default=100, ge=1, le=100, description="Image compression level (1-100)"
    )

    background: BackgroundType = Field(
        default=BackgroundType.AUTO, description="Background type for the image"
    )
...
class ImageEditingParams(BaseModel):
    ...
    image_id: str = Field(
        ..., description="Unique identifier of the image to edit", min_length=1
    )
    ...
    background: BackgroundType = Field(
        default=BackgroundType.AUTO, description="Background type for the edited image"
    )
